...
export default function SplashCursor({
  ...
  useGSAP(() => {
    ...
    gsap.fromTo(
      canvasRef.current,
      { opacity: 0 },
      { duration: 0.32, ease: 'power1.out', opacity: 1 },
    )
  }, { scope: rootRef })
  ...
  useEffect(() => {
    ...
    function updateFrame(timestamp: number) {
      ...
      render(null);
      frameCount += 1
      const sampleDuration = timestamp - sampleStarted
      if (sampleDuration >= 1000) {
        canvas!.dataset.fluidFps = String(Math.round((frameCount * 1000) / sampleDuration))
        frameCount = 0
    ...
    function hexToRGB(hex: string): ColorRGB {
      ...
      return { r: r * 0.38, g: g * 0.38, b: b * 0.38 };
    }

    let paletteIndex = 0
    function generateColor(): ColorRGB {
      const color = hexToRGB(ATLAS_DYE_COLORS[paletteIndex])
      paletteIndex = (paletteIndex + 1) % ATLAS_DYE_COLORS.length
      return color
    }

    let hasPointerPosition = false
    let splatCount = 0
    let pointerColor = generateColor()
  ...
  return (
    <div
      ref={rootRef}
      className={`splash-cursor ${className}`.trim()}
      data-fluid-cursor
      aria-hidden="true"
    >
      <canvas
